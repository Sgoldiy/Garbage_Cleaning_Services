...
class Complaint3ViewModel : ViewModel() {
    private val firestore = FirebaseFirestore.getInstance()
    private val _complaints = MutableLiveData<List<Complaint3>>() // Fix the type here
    val complaints: LiveData<List<Complaint3>> = _complaints
    ...
    fun fetchComplaints() {
        ...
        // For example:
        firestore.collection("complaintsOfPickupService")
            .get()
            .addOnSuccessListener { documents ->
                val complaintsList = mutableListOf<Complaint3>()
                for (document in documents) {
                    val complaintText = document.getString("complaint") ?: ""
                    val userEmail = document.getString("userEmail") ?: ""

                    val complaint = Complaint3(
